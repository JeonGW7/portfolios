...
public class FreeController {
	...
	public String content(Model model,String free_no,String pageNum) {
		...
		reply_list=sqlSession.selectList("reply.getReply",free_no1);

		sqlSession.update("free.readCount",free_no1);//조회수증기

		FreeDto freeDto=sqlSession.selectOne("free.getFree",new Integer(free_no));
        
		int  reply_free_no=0;
		//int reply_free_no=sqlSession.selectOne("reply.replyFreeNo",new Integer(free_no));//######### 답글
		//System.out.println("글내용 보기 컨트롤러 reply_free_no = "+reply_free_no);
		 
		
		model.addAttribute("freeDto",freeDto);//jsp에서 사용할데이터
		// model.addAttribute("reply_list",reply_list);//jsp에서 사용할데이터
		model.addAttribute("free_no",free_no1);
		model.addAttribute("pageNum",pageNum);
		model.addAttribute("list",list);
		model.addAttribute("reply",reply_list);
		//model.addAttribute("reply_free_no",reply_free_no);
		//  model.addAttribute("")
	...
	public String writePro(ReplyDto replyDto, String pageNum, Model model) throws Exception{
	...
	public String deletePro(Model model,String reply_rno, String free_no,String pageNum) {
	       
		if(reply_rno==null || reply_rno=="") {
        	reply_rno="0";
        }
        
        int reply_rno_su=Integer.parseInt(reply_rno);
        int free_no2=Integer.parseInt(free_no);
        
		if(reply_rno_su != 0) {//답글,원글 삭제
			Map<String, Integer>map=new HashMap<String, Integer>();
			
			map.put("free_no", new Integer(free_no2));
			
			sqlSession.delete("reply.replyDelete2",map);//free-no 모두 답글삭제
		}

		sqlSession.delete("free.freeDelete", free_no2);//원글삭제
	...
	//DB 댓글삭제
	   ...
	   public String replydeletePro(Model model,String reply_rno,String free_no,String pageNum) {
	      ...
	      sqlSession.delete("reply.replyDelete1",map);
	      model.addAttribute("pageNum",pageNum);
