...
import com.example.memboard.service.BoardService;

@Controller
public class BoardController {

   @Resource(name="com.example.memboard.service.BoardService")
   ...
   public String writePro(@ModelAttribute("boardDto") BoardDto boardDto, 
      ...
      if(boardDto.getNum() !=0 ) {//답글이면
         //답글 위치 확보
         boardService.reStep(boardDto);//답글 위치 확보
   ...
   public String content(Model model,String num,String pageNum) throws Exception{
      ...
      boardService.readcountDao(Integer.parseInt(num));//조회수 증가 ########

      BoardDto bdto=boardService.getBoard(Integer.parseInt(num));
